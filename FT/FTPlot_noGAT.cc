...
int FTPlot_noGAT(int start, int end, int channel) 
{
  // gROOT->ProcessLine("#include<complex>");
   int startRun = start;
   int endRun = end;
   int selectedCh = channel;
   int currentRun = startRun;
   char ftFileName[300], histFileName[300];

   if(startRun != endRun) { sprintf(ftFileName, "/global/project/projectdirs/majorana/users/jgruszko/FT/FTWF/%d_to_%d.root", startRun, endRun); }
   else{ sprintf(ftFileName, "/global/project/projectdirs/majorana/users/jgruszko/FT/FTWF/%d.root", startRun); }
...
//TFile *builtFile = new TFile("$MJDDATADIR/surfprot/data/built/P3END/OR_run45001822.root");
 ...
 //  vector<double>* timestamp= 0;
   ...
   FTTree->SetBranchAddress("channel", &ch);
...
*/
  ...
  // else{ nentries = nentriesGAT; }
   ...
   sprintf(selection, "channel == %d", selectedCh);
...
//cout<<"Got Entry 0 in both trees."<<endl;
   ...
   int entriesPass = (int) FTTree->Draw("channel", selection, "goff");
...
{
    if(argc < 4){
        cout << "Usage: " << argv[0] << " 'starting run number' 'ending run number' 'channel'." << endl;
        return 1;
    }
    ...
    return FTPlot_noGAT(startRun, endRun, channel);
}
